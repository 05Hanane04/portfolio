function Marquee() {
  return (

<section className="marquee-section">

<div className="marquee">

<div className="marquee-track">

<div className="marquee-group">

<span><i className="fa-brands fa-react"></i></span>
<span><i className="fa-brands fa-js"></i></span>
<span><i className="fa-brands fa-php"></i></span>
<span><i className="fa-brands fa-laravel"></i></span>
<span><i className="fa-brands fa-symfony"></i></span>
<span><i className="fa-solid fa-database"></i></span>
<span><i className="fa-brands fa-html5"></i></span>
<span><i className="fa-brands fa-css3-alt"></i></span>
<span><i className="fa-brands fa-python"></i></span>
<span><i className="fa-brands fa-bootstrap"></i></span>
<span><i className="fa-brands fa-github"></i></span>
<span><i className="fa-brands fa-figma"></i></span>

</div>

<div className="marquee-group">

<span><i className="fa-brands fa-react"></i></span>
<span><i className="fa-brands fa-js"></i></span>
<span><i className="fa-brands fa-php"></i></span>
<span><i className="fa-brands fa-laravel"></i></span>
<span><i className="fa-brands fa-symfony"></i></span>
<span><i className="fa-solid fa-database"></i></span>
<span><i className="fa-brands fa-html5"></i></span>
<span><i className="fa-brands fa-css3-alt"></i></span>
<span><i className="fa-brands fa-python"></i></span>
<span><i className="fa-brands fa-bootstrap"></i></span>
<span><i className="fa-brands fa-github"></i></span>
<span><i className="fa-brands fa-figma"></i></span>

</div>

</div>

</div>

</section>

  );
}

export default Marquee;